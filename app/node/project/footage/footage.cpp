        ...
        for (int i=0; i<footage_info.GetVideoStreams().size(); i++) {
          ...
          if (i < InputArraySize(kVideoParamsInput)) {
            ...
            if (colorspace.compare(default_rule_space) == 0) {
              if (existing.format() == VideoParams::Format::kFormatUnsigned8 ||
                  existing.format() == VideoParams::Format::kFormatUnsigned16) {
                colorspace = color_manager->GetDefaultByteInputColorSpace();
              } else if (existing.format() == VideoParams::Format::kFormatFloat16 ||
                         existing.format() == VideoParams::Format::kFormatFloat32) {
                colorspace = color_manager->GetDefaultFloatInputColorSpace();
              }
            }

            existing.set_colorspace(colorspace);

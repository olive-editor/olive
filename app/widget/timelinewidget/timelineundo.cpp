  ...
  point_(point)
{
  ...
  TransitionBlock* transition = block_->out_transition();
  if (transition) {
  ...
  for (int i=0;i<times.size();i++) {
    ...
    for (int j=0;j<blocks.size();j++) {
      ...
      if (b->in() < time && b->out() > time) {
        BlockSplitCommand* split_command = new BlockSplitCommand(b->track(), b, time, this);
    ...
    if (max_ripple_length > 0) {
      foreach (Block* resize, blocks_around_range) {
        if (resize->length() == max_ripple_length) {
          ...
          TrackRippleRemoveBlockCommand* remove_command = new TrackRippleRemoveBlockCommand(resize->track(), resize);
          remove_command->redo();
...
void TrackListInsertGaps::redo_internal()
{
  ...
  foreach (Block* block, blocks_to_append_gap_to) {
    ...
    block->track()->InsertBlockAfter(gap, block);
...
void TrackListInsertGaps::undo_internal()
{
  ...
  foreach (GapBlock* gap, gaps_added_) {
    gap->track()->RippleRemoveBlock(gap);
    gap->setParent(&memory_manager_);
  }
  gaps_added_.clear();
